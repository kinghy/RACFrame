...
static NSString *strValueKey = @"UITextView+RFKeyBoard_autoAdjustCoveredEditingstrValueKey";
static NSString *strShowKey = @"UITextView+RFKeyBoard_autoAdjustCoveredShow";
static NSString *strHideKey = @"UITextView+RFKeyBoard_autoAdjustCoveredHide";
static NSString *strbeginKey = @"UITextView+RFKeyBoard_autoAdjustCoveredEditingDidBegin";

@implementation UITextView (RFKeyBoard)
-(void)autoAdjustCoveredWithDelegate:(id<UITextViewDelegate>)delegate{
    ...
    if(beginSignal==nil){
        beginSignal = [(NSObject*)self.delegate rac_signalForSelector:@selector(textViewDidBeginEditing:) fromProtocol:@protocol(UITextViewDelegate)];
...
-(void)adjustWithRect:(CGRect)keyboardRect{
    if ([self isFirstResponder] && keyboardRect.origin.y>0) {
        ...
        if ((absRect.origin.y+absRect.size.height + 5 + frame.origin.y) > (screenView.frame.size.height-keyboardRect.size.height) || frame.origin.y!=0) {
            const float movementDuration = 0.3f; // tweak as needed
            int movement = (screenView.frame.size.height -keyboardRect.size.height) - ( absRect.origin.y+absRect.size.height + 5 + frame.origin.y);
